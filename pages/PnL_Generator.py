...
def get_font(size, bold=False):
    """
    Prioritas font:
    1. PublicSans-Bold.ttf dari root project (untuk semua teks utama)
    2. Fallback ke system fonts
    3. Default PIL font kalau semua gagal
    """
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))

    # Cari PublicSans-ExtraBold di beberapa lokasi
    public_sans_candidates = [
        os.path.join(BASE_DIR, "..", "PublicSans-Bold.ttf"),  # root project
        os.path.join(BASE_DIR, "PublicSans-Bold.ttf"),        # folder pages
        "PublicSans-Bold.ttf",                                 # working dir
    ]
    for p in public_sans_candidates:
        if os.path.exists(p):
            return ImageFont.truetype(p, size)

    # Fallback ke system fonts
    fallback_paths = [
...
def generate_pnl(emiten, harga_beli, harga_jual):
    ...
    # ── Warna ──
    GREEN = (6, 171, 109)    # #06ab6d — warna asli
    ...
    f_persen   = get_font(380, bold=True)
