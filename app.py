...
def load_logo(path="logo.png"):
    """Load logo sebagai base64 biar tidak ada render aneh dari Streamlit"""
    p = Path(path)
    if p.exists():
        with open(p, "rb") as f:
...
def filter_hari(df, hari):
    """Ganti .last() yang deprecated dengan iloc"""
    if hari >= 9999:
        return df
...
if logo_src:
    logo_html = f'<img src="{logo_src}" style="width:72px;height:72px;object-fit:contain;border-radius:10px;display:block;margin-left:auto;">'
else:
    logo_html = '<div style="width:72px;height:72px;border:1px dashed #30363d;border-radius:10px;display:flex;align-items:center;justify-content:center;color:#8b949e;font-size:0.65rem;margin-left:auto;">LOGO</div>'

st.markdown(f"""
<div style="display:flex;justify-content:space-between;align-items:center;padding:1rem 0 0.5rem;">
    <div>
        <div style="font-family:'Space Mono',monospace;font-size:1.4rem;font-weight:700;letter-spacing:-0.02em;color:#e6edf3;line-height:1.3;">
...
with col_kiri:
    ...
    fig_gauge = go.Figure(go.Indicator(
        mode  = "gauge+number",
        value = skor,
        number = {"font": {"size": 52, "color": warna, "family": "Space Mono"}, "suffix": ""},
        gauge = {
        ...
        for period_label, days in [("Yesterday", 1), ("Last Week", 7), ("Last Month", 30)]:
            try:
                ...
                <div class="metric-card" style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="color:#8b949e;font-size:0.83rem;">{period_label}</span>
                    <span class="badge" style="background:{w}22;color:{w};border:1px solid {w}55;">{l} &mdash; {s}</span>
    ...
    if len(df_hist) > 0:
        # FIX: pakai filter_hari() bukan .last()
        df_year  = filter_hari(df_hist, 365)
        idx_high = df_year["skor"].idxmax()
        idx_low  = df_year["skor"].idxmin()
        s_high   = float(df_year["skor"].max())
        s_low    = float(df_year["skor"].min())

        for lbl, s_val, idx in [("Yearly High", s_high, idx_high), ("Yearly Low", s_low, idx_low)]:
            w   = warna_skor(s_val)
            l   = label_skor(s_val)
            tgl = pd.Timestamp(idx).strftime("%b %d, %Y")
            st.markdown(f"""
            <div class="metric-card" style="display:flex;justify-content:space-between;align-items:center;">
                <span style="color:#8b949e;font-size:0.83rem;">{lbl}<br><span style="font-size:0.72rem;">({tgl})</span></span>
                <span class="badge" style="background:{w}22;color:{w};border:1px solid {w}55;">{l} &mdash; {s_val}</span>
            </div>""", unsafe_allow_html=True)
    ...
    <div class="desc-box">
        ...
        <p>
            ...
            Skor <b style="color:#e53e3e;">0–24</b> = Extreme Fear &nbsp;·&nbsp;
            ...
            Indeks ini bukan rekomendasi investasi. Gunakan sebagai salah satu referensi
...
with col_kanan:
    ...
    with tab1:
        periode  = st.radio("", ["30d", "90d", "1y", "All"], index=2, horizontal=True)
        hari_map = {"30d": 30, "90d": 90, "1y": 365, "All": 9999}
        ...
        for y, txt in [(12,"Extreme Fear"),(35,"Fear"),(50,"Neutral"),(65,"Greed"),(87,"Extreme Greed")]:
            fig.add_annotation(
                x=df_plot.index[-1], y=y, text=txt, showarrow=False,
                xanchor="right", font=dict(size=9, color="#8b949e")
            )
        ...
        fig.update_layout(
            ...
            yaxis=dict(gridcolor="#21262d", range=[0,100], title="Skor F&G"),
            yaxis2=dict(overlaying="y", side="right", showgrid=False, title="IHSG"),
            hovermode="x unified"
        )
    ...
    with tab2:
        ...
        for nama, skor_i, bbt, desc in indikator:
            w  = warna_skor(skor_i)
            lb = label_skor(skor_i)
            st.markdown(f"""
            <div class="metric-card">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                    <span style="font-weight:500;font-size:0.9rem;">{nama}</span>
                    <div>
                        <span class="badge" style="background:{w}22;color:{w};border:1px solid {w}55;margin-right:6px;">{lb}</span>
...
st.markdown(f"""
<div class="footer">
    <p>© {datetime.now().year} IHSG Fear & Greed Index &nbsp;·&nbsp; Data bersifat indikatif, bukan rekomendasi investasi &nbsp;·&nbsp; <a href="mailto:mugenworklabs@gmail.com" style="color:#8b949e;text-decoration:none;">mugenworklabs@gmail.com</a></p>
</div>
""", unsafe_allow_html=True)